<template>
  <div class="min-h-screen bg-gray-50">
    ...
    <div v-else-if="!product" class="flex flex-col items-center justify-center min-h-screen">
      ...
      <button @click="$router.push('/')" class="bg-blue-600 text-white px-6 py-3 rounded-lg hover:bg-blue-700 transition">
        Quay về trang chủ
      </button>
    </div>
    ...
    <div v-else class="container mx-auto px-4 py-8">
        ...
        <nav class="flex mb-8 text-sm">
          <router-link to="/" class="text-blue-600 hover:text-blue-800">Trang chủ</router-link>
          <span class="mx-2 text-gray-500">/</span>
          <router-link v-if="product.category_id" :to="`/category/${product.category_id._id}`" class="text-blue-600 hover:text-blue-800">
            {{ product.category_id.name }}
          </router-link>
          <span class="mx-2 text-gray-500">/</span>
          <span class="text-gray-500">{{ product.name }}</span>
        </nav>      <div class="grid grid-cols-1 lg:grid-cols-2 gap-12">
        <!-- Product Images -->
        <div class="space-y-4">
          <!-- Main Image -->
          ...
          <!-- Thumbnail Images -->
          <div v-if="product.images && product.images.length > 0" class="flex space-x-2 overflow-x-auto">
            <img 
              :src="getFullImage(product.main_image)"
              :alt="product.name"
              class="w-20 h-20 object-cover rounded-lg cursor-pointer border-2 transition"
              :class="selectedImage === getFullImage(product.main_image) ? 'border-blue-600' : 'border-gray-200'"
            ...
            <img 
              ...
              class="w-20 h-20 object-cover rounded-lg cursor-pointer border-2 transition"
              :class="selectedImage === getFullImage(image) ? 'border-blue-600' : 'border-gray-200'"
        ...
        <div class="space-y-6">
          ...
          <div>
            ...
            <div class="flex items-center space-x-4 mb-4">
              ...
              <div class="flex items-center space-x-1">
                ...
                <span class="text-gray-600">{{ product.views || 0 }} lượt xem</span>
              </div>
              ...
              <div class="flex items-center">
                ...
                <span v-if="product.sold > 0" class="text-gray-500 ml-2">• Đã bán {{ product.sold }}</span>
              </div>
            </div>
          </div>
          ...
          <div class="border-t border-b py-6">
            <div class="flex items-center space-x-4">
              <!-- Discounted Price -->
              <span v-if="product.discount_price" class="text-3xl font-bold text-red-600">
            ...
            <div class="flex space-x-4">
              <button 
                ...
                <svg v-else class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                  <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M3 3h2l.4 2M7 13h10l4-8H5.4m0 0L7 13m0 0l-2.5 5M7 13l3 3m0 0l3-3M10 16v2a1 1 0 002 0v-2m-2 0V6a1 1 0 112 0v10"/>
                </svg>
              ...
              <button 
                ...
                :class="isInWishlist ? 'text-red-600 border-red-300 bg-red-50' : 'text-gray-600'">
          ...
          <!-- Product Features -->
          <div v-if="product.tags && product.tags.length > 0" class="border-t pt-6">
      ...
      <div class="mt-16">
        ...
        <div class="py-8">
          ...
          <div v-if="activeTab === 'specifications'" class="space-y-4">
            ...
            <div v-if="product.specifications && Object.keys(product.specifications).length > 0" class="bg-white rounded-lg shadow overflow-hidden">
              <table class="w-full">
                <tbody class="divide-y divide-gray-200">
                  <tr v-for="(value, key) in product.specifications" :key="key" class="hover:bg-gray-50">
...
const fetchProduct = async (productId) => {
  ...
  try {
    ...
    if (response.data.category_id?._id) {
      const relatedResponse = await getRelatedProducts(response.data.category_id._id, productId)
