...
import { getDashboardStats, getGrowthAnalytics } from "@/api/admin/dashboardService";
  ...
  try {
    ...
    if (response && response.data && response.data.products) {
      ...
      inventory.value = response.data.products.map(product => ({
        ...
        category: product.category?.name || product.categoryName || "Không có danh mục",
    ...
    error.value = "Không thể tải dữ liệu kho hàng. " + (err.message || "Vui lòng thử lại.");
    inventory.value = [];
  } finally {
    loading.value = false;
  }
...
function calculateStats() {
  ...
  const totalInventoryValue = inventory.value.reduce((sum, i) => sum + (i.price * i.quantity), 0);
  const avgProductPrice = total ? (inventory.value.reduce((sum, i) => sum + i.price, 0) / total) : 0;
...
const filteredInventory = computed(() => {
  ...
  filtered.sort((a, b) => {
    ...
    if (sortField.value === 'name' || sortField.value === 'sku' || sortField.value === 'category') {
    ...
    <div class="space-y-6">
      ...
      <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        <div>
          ...
          <p class="text-gray-600 mt-1">Theo dõi và quản lý hàng tồn kho một cách hiệu quả</p>
        </div>
      ...
      <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
        <div class="bg-white p-6 rounded-2xl shadow-sm border border-gray-100 hover:shadow-md transition-all duration-300 group">
          <div class="flex items-center justify-between">
            <div>
              ...
              <p class="text-sm text-gray-500 mt-1">+{{ stats.growthRate }}% tăng trưởng</p>
            </div>
        ...
        <div class="bg-gradient-to-br from-green-50 to-emerald-50 p-6 rounded-2xl shadow-sm border border-green-100 hover:shadow-md transition-all duration-300 group">
          <div class="flex items-center justify-between">
            <div>
              <p class="text-sm font-medium text-green-700">Tổng giá trị kho</p>
              <p class="text-3xl font-bold text-green-900 mt-2">{{ formatCurrency(stats.totalInventoryValue) }}</p>
              <p class="text-sm text-green-600 mt-1">Giá trung bình: {{ formatCurrency(stats.avgProductPrice) }}</p>
            </div>
            <div class="w-12 h-12 bg-green-200 rounded-xl flex items-center justify-center group-hover:bg-green-300 transition-colors">
        ...
        <div class="bg-gradient-to-br from-yellow-50 to-amber-50 p-6 rounded-2xl shadow-sm border border-yellow-100 hover:shadow-md transition-all duration-300 group">
          <div class="flex items-center justify-between">
            <div>
              <p class="text-sm font-medium text-yellow-700">Danh mục nhiều hàng nhất</p>
              <p class="text-3xl font-bold text-yellow-900 mt-2">{{ stats.topCategory || 'Không có' }}</p>
              <p class="text-sm text-yellow-600 mt-1">Còn hàng: {{ stats.inStock }} | Sắp hết: {{ stats.lowStock }}</p>
            </div>
            <div class="w-12 h-12 bg-yellow-200 rounded-xl flex items-center justify-center group-hover:bg-yellow-300 transition-colors">
        ...
        <div class="bg-gradient-to-br from-red-50 to-pink-50 p-6 rounded-2xl shadow-sm border border-red-100 hover:shadow-md transition-all duration-300 group">
          <div class="flex items-center justify-between">
            <div>
              <p class="text-sm font-medium text-red-700">Đơn hàng & Đã bán</p>
              <p class="text-3xl font-bold text-red-900 mt-2">{{ stats.totalOrders }} / {{ stats.totalSold }}</p>
              <p class="text-sm text-red-600 mt-1">Hết hàng: {{ stats.outOfStock }}</p>
            </div>
            <div class="w-12 h-12 bg-red-200 rounded-xl flex items-center justify-center group-hover:bg-red-300 transition-colors">
      ...
      <div class="bg-white p-4 sm:p-6 rounded-2xl shadow-sm border border-gray-100">
        ...
        <div class="flex flex-wrap gap-2 mt-4 pt-4 border-t border-gray-100">
          ...
          <button 
            @click="selectedStatus = 'all'; selectedCategory = 'all'; searchQuery = ''"
      ...
      <div class="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden">
        ...
        <div v-if="loading" class="p-6">
          <div class="animate-pulse">
            ...
            <div class="space-y-3">
              <div v-for="n in 5" :key="n" class="flex items-center space-x-4 py-3">
              ...
              <tr>
                ...
                >
                  <div class="flex items-center gap-1">
                    Sản phẩm
                    <svg v-if="sortField === 'name'" :class="{'rotate-180': sortDirection === 'desc'}" class="w-3 h-3 transition-transform" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                ...
                >
                  <div class="flex items-center gap-1">
                    Danh mục
                    <svg v-if="sortField === 'category'" :class="{'rotate-180': sortDirection === 'desc'}" class="w-3 h-3 transition-transform" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                      <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M5 15l7-7 7 7"/>
                    </svg>
                  </div>
                </th>
                ...
                >
                  <div class="flex items-center gap-1">
                    Tồn kho
                    <svg v-if="sortField === 'quantity'" :class="{'rotate-180': sortDirection === 'desc'}" class="w-3 h-3 transition-transform" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                      <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M5 15l7-7 7 7"/>
                    </svg>
                  </div>
                </th>
                ...
                >
                  <div class="flex items-center gap-1">
                    Giá bán
                    <svg v-if="sortField === 'price'" :class="{'rotate-180': sortDirection === 'desc'}" class="w-3 h-3 transition-transform" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                      <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M5 15l7-7 7 7"/>
                    </svg>
                  </div>
                </th>
                ...
                >
                  <div class="flex items-center gap-1">
                    Cập nhật
                    <svg v-if="sortField === 'updatedAt'" :class="{'rotate-180': sortDirection === 'desc'}" class="w-3 h-3 transition-transform" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                      <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M5 15l7-7 7 7"/>
                    </svg>
                  </div>
                </th>
              ...
              >
                ...
                <td class="px-6 py-4 whitespace-nowrap">
                  <div class="flex items-center">
                    ...
                    <div class="ml-2 text-xs text-gray-500">đơn vị</div>
                  </div>
                </td>
                ...
                <td class="px-6 py-4 whitespace-nowrap">
                  <span
                    :class="{
                      'inline-flex px-3 py-1 text-xs font-medium rounded-full bg-green-100 text-green-800': getStockStatus(item.quantity) === 'Còn hàng',
                      'inline-flex px-3 py-1 text-xs font-medium rounded-full bg-yellow-100 text-yellow-800': getStockStatus(item.quantity) === 'Sắp hết',
                      'inline-flex px-3 py-1 text-xs font-medium rounded-full bg-red-100 text-red-800': getStockStatus(item.quantity) === 'Hết hàng',
                    }"
                  >
                ...
                <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                  <div class="flex justify-end gap-1">
                    <div class="relative group">
                      ...
                      <div class="absolute bottom-full right-0 mb-2 px-2 py-1 text-xs text-white bg-gray-900 rounded opacity-0 group-hover:opacity-100 transition-opacity whitespace-nowrap">
                        Chỉnh sửa
                      </div>
                    </div>
                    <div class="relative group">
        ...
        <div v-if="filteredInventory.length === 0" class="text-center py-12">
          <svg class="mx-auto h-12 w-12 text-gray-400" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M20 13V6a2 2 0 00-2-2H6a2 2 0 00-2 2v7m16 0v5a2 2 0 01-2 2H6a2 2 0 01-2 2v-5m16 0h-2.586a1 1 0 00-.707.293l-2.414 2.414A1 1 0 0014 21H10a1 1 0 01-.707-.293l-2.414-2.414A1 1 0 006.586 18H4"/>
          </svg>
          <h3 class="mt-2 text-sm font-medium text-gray-900">Không tìm thấy sản phẩm</h3>
          <p class="mt-1 text-sm text-gray-500">Thử thay đổi bộ lọc hoặc từ khóa tìm kiếm khác.</p>
        </div>

        <!-- Pagination -->
        <div v-if="filteredInventory.length > 0" class="px-6 py-4 border-t border-gray-200 bg-white">
          <div class="flex items-center justify-between">
            <!-- Thông tin hiển thị -->
