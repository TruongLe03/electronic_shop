<template>
  <div class="min-h-screen bg-gray-50">
    <div class="container mx-auto px-4 py-8">
      ...
      <button
        @click="router.back()"
        class="mb-4 hover:bg-gray-200 text-black py-2 px-4 rounded-lg font-semibold transition-colors flex items-center gap-2"
      >
        <svg
          class="w-5 h-5"
          fill="none"
      ...
      <div v-else-if="order" class="max-w-4xl mx-auto">
          ...
          >
            <div>
              ...
              <p class="text-gray-600">
                Mã đơn hàng: <span class="font-semibold">#{{ orderId }}</span>
              </p>
            </div>
            ...
            <div class="space-y-3">
              ...
              <div>
                ...
                <span
                  ...
                  class="inline-block px-3 py-1 rounded-full text-sm font-semibold ml-2"
                >
            ...
            >
              ...
              <div class="flex-1">
                ...
                <p class="text-sm text-gray-600">
                  {{ formatPrice(item.discount_price || item.price) }} x
                  {{ item.quantity }}
                </p>
              </div>
        ...
        <div class="bg-white rounded-lg shadow-md p-6 mb-6">
          ...
          <div class="space-y-2">
            <div class="flex justify-between text-gray-600">
              <span>Tạm tính:</span>
              <span>{{
                formatPrice(order.subtotal || calculateSubtotal())
              }}</span>
            </div>
            <div class="flex justify-between text-gray-600">
            ...
            <!-- Coupon Discount -->
            <div
              v-if="order.coupon_code && order.discount_amount"
        ...
        <div class="flex flex-col sm:flex-row gap-4">
          ...
          <button
            v-if="
              ['pending', 'payment_pending', 'payment_failed'].includes(
                order.status
              )
            "
            @click="cancelOrder"
          ...
          <router-link
            ...
            :to="`/payment/${orderId}`"
            class="flex-1 bg-orange-600 hover:bg-orange-700 text-white py-3 px-6 rounded-lg font-semibold text-center transition-colors"
          >
            Tiếp tục thanh toán
          </router-link>
        </div>
  ...
  <Teleport to="body">
    <div
      ...
      class="modal-overlay fixed inset-0 bg-opacity-50 flex items-center justify-center p-4"
      @click="showCancelModal = false"
    >
      <div class="bg-gray-100 rounded-lg shadow-xl w-full max-w-md" @click.stop>
        <div class="p-6">
...
const confirmCancelOrder = async () => {
  ...
  try {
    ...
    if (response.success) {
      ...
      order.value = response.data; // Update entire order object with response data
